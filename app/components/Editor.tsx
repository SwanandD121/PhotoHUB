...
export function TipTapEditor({json, setJson}: {json: JSONContent | null, setJson: any}) {
  const editor = useEditor({
    extensions: [StarterKit],
    content: json,
    editorProps: {
        attributes: {
            class: "focus:outline-none min-h-[150px] prose prose-sm md:prose-base"
        }
    },
    onUpdate: () => {
  ...
  return (
    <div>
      ...
      <EditorContent editor={editor} className="border-2 rounded-lg p-2 min-h-[150px] mt-2 "/>
    </div>
  );
}
